...
```powershell
python -m pip install -r .\detector_module\requirements.txt
```

## 一键运行

```powershell
powershell -ExecutionPolicy Bypass -File .\detector_module\run_baseline.ps1
```

可调整训练轮数和批大小：

```powershell
.\detector_module\run_baseline.ps1 -Epochs 100 -BatchSize 16
```
...
```powershell
python -m detector_module.evaluate_detector `
  --model .\detector_module\runs\yolov8n_baseline_v1\weights\submission_map50.pt
```

训练框架默认按照 mAP@0.5:0.95 保存 `best.pt`。如正式评分采用 mAP@0.5，需先在已保存权重中按照验证集 mAP@0.5 选择提交权重：

```powershell
python -m detector_module.select_baseline_checkpoint `
  --run .\detector_module\runs\yolov8n_baseline_v1
```

### 离线增广数据集对照实验

团队产出的增广数据集在 `E:\yolo_augmented`（训练 4400 张 = 595 张原图 + 3805 张派生图，
...
```powershell
# 对照组：595 张原图，60 轮
python -m detector_module.train_detector --data detector_module/configs/data_noaug.yaml `
  --epochs 60 --patience 999 --batch-size 16 --workers 2 --no-builtin-aug --name ab_noaug_e60

# 增广组：4400 张，60 轮（等 epoch）
python -m detector_module.train_detector --data detector_module/configs/data_augmented.yaml `
...
```powershell
python -m detector_module.compare_augmentation `
  --run 增广60轮=detector_module/runs/ab_augmented_e60 `
  --run 对照60轮=detector_module/runs/ab_noaug_e60 `
  --run 对照444轮=detector_module/runs/ab_noaug_e444 `
...
```powershell
python -m detector_module.export_detector `
  --model .\detector_module\runs\yolov8n_baseline_v1\weights\submission_map50.pt
```
...
```powershell
python -m detector_module.diagnose_detector `
  --model .\detector_module\runs\yolov8n_baseline_v1\weights\submission_map50.pt
```
...
```powershell
python -m detector_module.build_incremental_dataset
```
