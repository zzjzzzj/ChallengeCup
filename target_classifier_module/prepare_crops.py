...
from target_classifier_module.dataset import build_target_crop_dataset
...
def main() -> None:
    ...
    parser.add_argument(
        "--index",
        type=Path,
        default=Path("scene_module/artifacts/scene_index.csv"),
        help="包含image_path/split/sensor/scene字段的原图索引",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("target_classifier_module/artifacts/target_crops"),
    )
    parser.add_argument(
        "--padding-ratio",
        type=float,
