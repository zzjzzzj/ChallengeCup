...
```powershell
python -m target_classifier_module.train_whole_image `
  --manifest-dir detector_module/artifacts/detection_dataset/manifests `
  --epochs 12 `
  --batch-size 32 `
  --augmentation none `
  --output target_classifier_module/runs/resnet18_whole_image_baseline_none
```
...
```powershell
python -m target_classifier_module.infer `
  --image "已裁剪目标.png" `
  --checkpoint target_classifier_module/runs/resnet18_target_baseline_none/best.pt
```

## 5. ONNX与310B前置准备

```powershell
python -m target_classifier_module.export_classifier `
  --checkpoint target_classifier_module/runs/resnet18_target_baseline_none/best.pt `
  --output target_classifier_module/runs/resnet18_target_baseline_none/exports
```
