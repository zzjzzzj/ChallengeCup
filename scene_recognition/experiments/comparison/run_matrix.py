...
def _resnet_crop(init: str, epochs: int, seed: int, output_root: Path) -> Experiment:
    ...
    argv = [
        "-m", "scene_recognition.target_classifier_module.train_classifier",
        "--epochs", str(epochs),
        "--batch-size", "32",
        "--image-size", "224",
        "--seed", str(seed),
...
def _resnet_whole(init: str, epochs: int, seed: int, output_root: Path) -> Experiment:
    ...
    out = output_root / name
    argv = [
        "-m", "scene_recognition.target_classifier_module.train_whole_image",
...
def _yolo_detect(init: str, epochs: int, seed: int, output_root: Path) -> Experiment:
    ...
    argv = [
        "-m", "scene_recognition.detector_module.train_detector",
        "--epochs", str(epochs),
        "--patience", "20",
        "--batch-size", "16",
        "--image-size", "640",
