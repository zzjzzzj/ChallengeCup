...
from image_processing.scene_runtime import DEFAULT_POLICY
...
_POLICY_EXTENSIONS = {
    ...
    "uncertain": {
        "sensor_weights": {"visible": 0.34, "ir": 0.33, "sar": 0.33},
        "feature_weights": {"intensity": 0.25, "texture": 0.25, "frequency": 0.25, "detail": 0.25},
    },
}


def _build_scene_policy() -> dict[str, dict[str, Any]]:
    policy: dict[str, dict[str, Any]] = {}
    for scene, base in DEFAULT_POLICY.items():
        merged = dict(base)
        merged.update(_POLICY_EXTENSIONS.get(scene, _POLICY_EXTENSIONS["uncertain"]))
        policy[scene] = merged
    return policy


SCENE_POLICY = _build_scene_policy()
            ...
            invalid.append(
                {
                    ...
                    "reason": (
                        f"{TARGET_CN.get(box.class_name, box.class_name)} is not expected in "
                        f"{SCENE_CN.get(scene_label, scene_label)} scene"
                    ),
                }
            )
    return invalid
...
def resolve_final_scene(
    ...
    details["status"] = "conflict" if invalid else "consistent"
...
def build_consistency_report(
    ...
    return {
        ...
        "rule": (
            "scene + target must match dataset-realistic combinations; "
            "impossible pairs are counted as L_proto/L_cls penalties."
        ),
    }
...
def active_experts(modality: str, detections: list[DetectionBox], final_scene: str) -> list[str]:
    ...
    if not targets:
        fallback = SCENE_POLICY.get(final_scene, SCENE_POLICY.get("urban", {}))
        targets = list(fallback.get("priority_classes", []))
    experts = []
...
def build_decision(
    ...
    policy = dict(SCENE_POLICY.get(final_scene.label, SCENE_POLICY.get("urban", {})))
    if isinstance(environment.get("policy_seed"), dict):
        for key, value in environment["policy_seed"].items():
            policy.setdefault(key, value)

    threshold = float(policy.get("confidence_threshold", 0.35))
    ...
    priority_base = list(policy.get("priority_classes", []))
    if selected_targets:
        priority_classes = sorted(set(priority_base) | set(selected_targets))
    else:
        priority_classes = priority_base

    return {
        "detector_profile": policy.get("detector_profile", "general"),
        "confidence_threshold": round(threshold, 4),
        "priority_classes": priority_classes,
        "sensor_weights": policy.get("sensor_weights", {}),
        "feature_weights": policy.get("feature_weights", {}),
        "enhancement": policy.get("enhancement")
        or environment.get("policy_seed", {}).get("enhancement"),
        "notes": policy.get("notes"),
        "policy_source": "image_processing.scene_runtime.DEFAULT_POLICY",
        "expert_routing": {
