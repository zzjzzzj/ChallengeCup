...
```powershell
python .\scene_module\analyze_and_prepare.py `
  --dataset 'E:\BaiduNetdiskDownload\datasets_r1_base_train\datasets_r1_base_train' `
  --output .\scene_module\artifacts

python .\scene_module\feature_engineering.py extract `
  --index .\scene_module\artifacts\scene_index.csv `
  --output .\scene_module\artifacts\scene_features.csv

python .\scene_module\feature_engineering.py evaluate `
  --features .\scene_module\artifacts\scene_features.csv `
  --output .\scene_module\runs\feature_baseline
```

单图推理：

```powershell
python .\scene_module\feature_infer.py `
  --image '待识别图像.png' `
  --model .\scene_module\runs\feature_baseline\scene_feature_svm.joblib `
  --metadata .\scene_module\runs\feature_baseline\model_metadata.json
```
