...
default_initial_model_path() {
  local name
  local resolved
  for name in "${MODEL_NAME}" yolo26n.pt yolov8n.pt best.pt; do
    if resolved="$(resolve_model_file "${name}")"; then
      printf '%s\n' "${resolved}"
      return 0
    fi
  done
...
if [[ "${RUN_TRAIN}" -eq 1 && -z "${INITIAL_MODEL}" ]]; then
  if INITIAL_MODEL="$(default_initial_model_path)"; then
    ...
    fail "missing --initial-model or --private-root
$(initial_model_hint)"
  fi
fi
...
if [[ "${RUN_TRAIN}" -eq 1 ]]; then
  case "${INITIAL_MODEL}" in
    *.onnx|*.om)
      fail "initial model must be a trainable .pt/.yaml model, not deployment model: ${INITIAL_MODEL}
$(initial_model_hint)"
      ;;
  esac
  if resolved_model="$(resolve_model_file "${INITIAL_MODEL}")"; then
