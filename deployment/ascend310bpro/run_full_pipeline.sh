...
Default flow:
  ...
  2. Optionally run Class-IL incremental training.
  3. Convert ONNX models to cached OM files.
  4. Run NPU inference.
  5. Build Agent-style JSON reports, CSV summary, and memory JSONL.

Common options:
  --data PATH                  Original YOLO dataset root or data.yaml.
  --workspace PATH             Pipeline workspace. Default: outputs/ascend310bpro_full.
  ...
  --default-scene air|sea|urban|forest
                               Used when predictions do not contain a scene result.
...
Stage controls:
  ...
  --report-only                Skip earlier stages; format existing predictions only.
  ...
  --skip-report                Do not build Agent-style report outputs.
  --no-memory                  Do not write Agent memory JSONL.
  --append-memory              Append to existing memory instead of rewriting this run.
  --include-modality           Include modality text in report descriptions.
...
DATA_PATH=""
WORKSPACE="${PROJECT_ROOT}/outputs/ascend310bpro_full"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --output-dir)
      ...
      ;;
    --report-dir)
      REPORT_OUTPUT="${2:?missing value for $1}"
      shift 2
      ;;
    --predictions)
      PREDICTIONS_PATH="${2:?missing value for $1}"
      shift 2
      ;;
    --memory)
      MEMORY_PATH="${2:?missing value for $1}"
      shift 2
      ;;
    --soc-version)
    ...
    --default-modality)
      ...
      ;;
    --default-scene)
      DEFAULT_SCENE="${2:?missing value for $1}"
      shift 2
      ;;
    --split)
...
case "${DEFAULT_SCENE}" in
  ""|air|sea|urban|forest) ;;
  *) fail "--default-scene must be one of: air, sea, urban, forest" ;;
esac
...
[[ -n "${REPORT_OUTPUT}" ]] || REPORT_OUTPUT="${WORKSPACE}/agent_reports"
[[ -n "${MEMORY_PATH}" ]] || MEMORY_PATH="${WORKSPACE}/agent_memory.jsonl"
if [[ -z "${PREDICTIONS_PATH}" ]]; then
  PREDICTIONS_PATH="${INFER_OUTPUT}/predictions.jsonl"
fi
...
if [[ "${RUN_REPORT}" -eq 1 && "${RUN_INFER}" -eq 0 && ! -f "${PREDICTIONS_PATH}" ]]; then
  fail "predictions not found for report stage: ${PREDICTIONS_PATH}"
fi
...
if [[ "${RUN_AUGMENT}" -eq 1 ]]; then
  ...
  echo "[INFO] Stage 1/5: dataset augmentation"
  "${augment_cmd[@]}"
elif [[ -f "${AUGMENTED_DATA}/data.yaml" ]]; then
  echo "[INFO] Stage 1/5 skipped: reuse augmented dataset ${AUGMENTED_DATA}/data.yaml"
else
  echo "[INFO] Stage 1/5 skipped: no augmented dataset check requested"
fi
...
if [[ "${RUN_TRAIN}" -eq 1 ]]; then
  ...
  echo "[INFO] Stage 2/5: optional Class-IL training"
  "${training_cmd[@]}"
else
  echo "[INFO] Stage 2/5 skipped: board-side training is optional"
fi
...
if [[ "${RUN_INFER}" -eq 1 ]]; then
  ...
  if [[ -n "${SINGLE_MODEL}" ]]; then
    echo "[INFO] Stage 4/5: single six-class best.onnx inference"
  else
    echo "[INFO] Stage 4/5: routed inference, scene recognition -> model matching"
  fi
  "${infer_cmd[@]}"
  echo "[INFO] Inference summary: ${INFER_OUTPUT}/summary.json"
  echo "[INFO] Inference JSONL  : ${INFER_OUTPUT}/predictions.jsonl"
else
  echo "[INFO] Stage 4/5 skipped: inference disabled"
fi
...
if [[ "${RUN_REPORT}" -eq 1 ]]; then
  ...
  if [[ -f "${INFER_OUTPUT}/summary.json" && "${PREDICTIONS_PATH}" == "${INFER_OUTPUT}/predictions.jsonl" ]]; then
    report_cmd+=(--summary "${INFER_OUTPUT}/summary.json")
  elif [[ -f "${PREDICTIONS_PATH%/*}/summary.json" ]]; then
    report_cmd+=(--summary "${PREDICTIONS_PATH%/*}/summary.json")
  fi
  if [[ "${NO_MEMORY}" -eq 0 ]]; then
    report_cmd+=(--memory "${MEMORY_PATH}")
    if [[ "${APPEND_MEMORY}" -eq 0 ]]; then
      report_cmd+=(--rewrite-memory)
    fi
  else
    report_cmd+=(--no-memory)
  fi
  if [[ -n "${DEFAULT_MODALITY}" ]]; then
    report_cmd+=(--default-modality "${DEFAULT_MODALITY}")
  fi
  if [[ -n "${DEFAULT_SCENE}" ]]; then
    report_cmd+=(--default-scene "${DEFAULT_SCENE}")
  fi
  if [[ "${INCLUDE_MODALITY}" -eq 1 ]]; then
    report_cmd+=(--include-modality)
  fi

  echo "[INFO] Stage 5/5: Agent-style output reports"
  "${report_cmd[@]}"
else
  echo "[INFO] Stage 5/5 skipped: report output disabled"
fi
